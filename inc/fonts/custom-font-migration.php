...
add_action( 'init', 'migrate_zbauerarchitect_custom_fonts', 99 );

function migrate_zbauerarchitect_custom_fonts() {
	// The data has already been transformed
	if ( get_theme_mod( 'zbauerarchitect_legacy_font_settings' ) ) {
		return;
	}
	...
	if ( isset( $font_families['custom'] ) && is_array( $font_families['custom'] ) ) {
		...
		set_theme_mod( 'zbauerarchitect_legacy_font_settings', true );
		return;
	}

	// Look first for fonts customized via Customizer, then for fonts configured in the child theme.json "the old way"
	// Also count fonts registerd to the zbauerarchitect font provider
