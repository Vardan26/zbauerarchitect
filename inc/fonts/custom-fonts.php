...
add_filter( 'jetpack_google_fonts_list', 'zbauerarchitect_filter_jetpack_google_fonts_list' );

$zbauerarchitect_enqueued_font_slugs = array();
...
/**
 * Build a list of all font slugs provided by zbauerarchitect from theme.json
 *
 * @return array Collection of all font slugs defined in the theme.json file
 */
function collect_fonts_from_zbauerarchitect() {
	$fonts           = array();
		...
		if ( is_string( $theme_json ) ) {
			...
			if ( is_array( $parent_theme_json_data ) && isset( $parent_theme_json_data['settings']['typography']['fontFamilies'] ) ) {
				...
				foreach ( $font_families as $font ) {
					...
					if ( array_key_exists( 'provider', $font ) && 'zbauerarchitect-fonts' === $font['provider'] ) {
						$fonts[] = $font;
					}
				}
			}
...
function enqueue_global_styles_fonts() {
	...
	global $zbauerarchitect_enqueued_font_slugs;

	$font_slugs = array();
	$font_css   = '';

	if ( is_admin() ) {
		$font_families = collect_fonts_from_zbauerarchitect();
	...
	$zbauerarchitect_enqueued_font_slugs = $font_slugs;
	...
	wp_register_style( 'zbauerarchitect_font_faces', '' );
	wp_enqueue_style( 'zbauerarchitect_font_faces' );

	// Add the styles to the stylesheet.
	wp_add_inline_style( 'zbauerarchitect_font_faces', $font_css );
}

/**
 * Enqueue all of the fonts provided by zbauerarchitect for FSE use
 */
function enqueue_fse_font_styles( $fonts ) {
	$fonts    = collect_fonts_from_zbauerarchitect();
	$font_css = '';
...
function enqueue_block_fonts( $content, $parsed_block ) {
	...
	if ( ! is_admin() && isset( $parsed_block['attrs']['fontFamily'] ) ) {
		...
		if ( ! in_array( $font_slug, $zbauerarchitect_enqueued_font_slugs, true ) ) {
			$font_css = get_style_css( $font_slug );
			if ( $font_css ) {
				$zbauerarchitect_enqueued_font_slugs[] = $font_slug;
				wp_add_inline_style( 'zbauerarchitect_font_faces', $font_css );
			}
		}
	}
 ...
 * This filters out all of the fonts zbauerarchitect has already registered.
 */
function zbauerarchitect_filter_jetpack_google_fonts_list( $jetpack_fonts ) {
	$theme_fonts         = collect_fonts_from_zbauerarchitect();
